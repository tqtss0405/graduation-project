...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    // Lấy tất cả sản phẩm đang bán nằm trong khoảng giá (cũ - không dùng phân trang)
    List<Product> findByActiveTrueAndPriceBetween(Double finalMinPrice, Double finalMaxPrice);
 
    // Lấy sản phẩm đang bán, THEO DANH MỤC và nằm trong khoảng giá (cũ - không dùng phân trang)
    List<Product> findByCategoryAndActiveTrueAndPriceBetween(Category category, Double finalMinPrice, Double finalMaxPrice);
    ...
    List<Product> findByCategoryAndActiveTrueAndPriceBetween(Category category, BigDecimal minPrice, BigDecimal maxPrice, Sort sort);
